...
class RippleTransitionLayout: UICollectionViewLayout {
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        let c123 = barycentricOf(collectionView!.viewPortCenter, P1: layoutP1.centerPosition, P2: layoutP2.centerPosition, P3: layoutP3.centerPosition)
...
//        return CGSize(width: CGFloat(2 + ytCols) * layoutP1.template.width, height: CGFloat(3 + ytRows) * layoutP1.template.height)
    ...
    func toggleTemplatFor(scene: SceneState) {
        var template: Template? = nil
        switch scene {
            case .surfing: template = Template.surf
            case .watching: template = Template.watch
            default: template = Template.watch
        }
        layoutP1.template = template!
        layoutP2.template = template!
        layoutP2.centerPosition = layoutP1.centerOf(layoutP2.center)
        layoutP3.template = template!
        layoutP3.centerPosition = layoutP1.centerOf(layoutP3.center)
        invalidateLayout()
    }
